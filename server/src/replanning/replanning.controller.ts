...
export class ReplanningController {
  ...
  @Post('observe') async observe(@Body() body: any) {
    ...
    const result =
      hasRuntimePayload && Array.isArray(body.events)
        ? await this.service.observeExternalEvents(
            body.previousContext,
            body.currentContext,
            body.itinerary,
            body.events,
        ...
        : hasRuntimePayload
          ? await this.service.observeRuntime(
              body.previousContext,
              body.currentContext,
              body.itinerary,
            )
    ...
    return localizeVisitorPayload(
      result,
      normalizeVisitorLocale(body.locale),
    );
  }
