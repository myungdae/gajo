...
describe('ConciergeService service-area handling', () => {
  ...
  it('invokes Journey Composer with both explicit Gajo destinations and never category discovery',async()=>{const requested=[{entityId:'https://gajo-wellness.kr/ontology#gajoHotSpringComplex',label:'백두산천지온천',requestedLabel:'가조온천',resolved:true,requested:true,source:'RDM'},{entityId:'https://gajo-wellness.kr/ontology#suseungdae',label:'수승대',requestedLabel:'수승대',resolved:false,requested:true,source:'SEMANTIC'}],contextService={createContext:jest.fn(async(input:any)=>({context:{contextNo:'RC-M',operationUri:'gajo:operation',regionId:'gajo',mustVisitPlaces:input.mustVisitPlaces},evidence:[],firedRules:[]}))},orchestrator={run:jest.fn(async(_id:string,_op:string,context:any)=>({tasks:[],executionLog:[],recommendation:{itinerary:{steps:context.mustVisitPlaces.map((x:any)=>({entityId:x.entityId,programLabel:x.label,requestedLabel:x.requestedLabel,actions:x.resolved?{detail:{}}:{}}))}}}))},discovery={resolveRequestedDestinations:jest.fn(async()=>requested),resolveReference:jest.fn(),discover:jest.fn()};const service=new ConciergeService(contextService as any,orchestrator as any,{label:jest.fn()}as any,{get:jest.fn(()=>GAJO_REGION_CONFIG),detectOutOfRegion:jest.fn()}as any,discovery as any);const result:any=await service.chat({regionId:'gajo',inputMode:'FREE_TEXT',rawMessage:'가조온천하고 수승대 가고 싶어요.'});expect(result).toMatchObject({intentRoute:'JOURNEY_PLAN',requestedDestinations:requested,visitorMessage:expect.stringContaining('가조온천과 수승대'),recommendation:{itinerary:{steps:[{programLabel:'백두산천지온천',requestedLabel:'가조온천'},{programLabel:'수승대',actions:{}}]}}});expect(contextService.createContext).toHaveBeenCalledWith(expect.objectContaining({mustVisitPlaces:requested}));expect(orchestrator.run).toHaveBeenCalled();expect(discovery.discover).not.toHaveBeenCalled();expect(JSON.stringify(result)).not.toContain('온천먹거리')});
});
