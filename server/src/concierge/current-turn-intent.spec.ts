...
describe('current-turn category continuity', () => {
  it('keeps an elliptical alternative in the current cafe discovery category', () => {
    expect(routeNaturalLanguageIntent({ rawMessage: '다른 곳은?', inputMode: 'FREE_TEXT', isFollowup: true, discoveryCategoryHint: 'CAFE' })).toEqual({ intentRoute: 'PLACE_DISCOVERY', category: 'CAFE' });
  });
