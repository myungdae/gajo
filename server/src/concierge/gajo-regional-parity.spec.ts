...
describe('Gajo shared regional concierge parity',()=>{
  ...
  it('keeps ambiguous 가조온천 as a non-operational place concept and preserves 수승대',async()=>{const resolver=new PlaceDiscoveryService(regional as any,undefined,undefined,undefined,new RegionConfigService());const destinations:any=await resolver.resolveRequestedDestinations('gajo',['가조온천','수승대']);expect(destinations).toEqual([expect.objectContaining({entityId:'https://gajo-wellness.kr/semantic#gajoHotSpringArea',label:'가조온천',requestedLabel:'가조온천',resolved:false,requested:true,source:'SEMANTIC',entityType:'PLACE_CONCEPT'}),expect.objectContaining({entityId:'https://gajo-wellness.kr/ontology#suseungdae',label:'수승대',resolved:false,requested:true,source:'SEMANTIC',verificationStatus:'UNVERIFIED'})]);expect(destinations.every((item:any)=>item.actions===undefined)).toBe(true)});
