...
describe('Gajo shared regional concierge parity',()=>{
  ...
  it('registers Gajo in the shared RDM-first dataset without Hapcheon entities',()=>{
    expect(REGIONAL_CANDIDATE_DATASETS.gajo.records).toHaveLength(6);
  ...
  it('keeps ambiguous 가조온천 non-operational and resolves review-only 수승대 without actions',async()=>{const resolver=new PlaceDiscoveryService(regional as any,undefined,undefined,undefined,new RegionConfigService());const destinations:any=await resolver.resolveRequestedDestinations('gajo',['가조온천','수승대']);expect(destinations).toEqual([expect.objectContaining({entityId:'https://gajo-wellness.kr/semantic#gajoHotSpringArea',label:'가조온천',requestedLabel:'가조온천',resolved:false,requested:true,source:'SEMANTIC',entityType:'PLACE_CONCEPT'}),expect.objectContaining({entityId:'https://gajo-wellness.kr/ontology#suseungdae',label:'수승대',resolved:true,requested:true,source:'RDM',verificationStatus:'UNKNOWN'})]);expect(destinations.every((item:any)=>item.actions===undefined)).toBe(true)});

  it('canonically resolves both representative destinations and their aliases without operational actions',async()=>{const resolver=new PlaceDiscoveryService(regional as any,undefined,undefined,undefined,new RegionConfigService());const destinations:any=await resolver.resolveRequestedDestinations('gajo',['창포원','수승대관광지']);expect(destinations).toEqual([expect.objectContaining({entityId:'https://gajo-wellness.kr/ontology#geochangChangpowon',label:'거창창포원',requestedLabel:'창포원',resolved:true,source:'RDM',verificationStatus:'UNKNOWN'}),expect.objectContaining({entityId:'https://gajo-wellness.kr/ontology#suseungdae',label:'수승대',requestedLabel:'수승대관광지',resolved:true,source:'RDM',verificationStatus:'UNKNOWN'})]);expect(destinations.every((item:any)=>item.latitude===undefined&&item.longitude===undefined&&item.actions===undefined)).toBe(true)});

  it.each([['수승대 근처 밥집은?','https://gajo-wellness.kr/ontology#suseungdae','FOOD','미가추어탕'],['창포원 근처 카페는?','https://gajo-wellness.kr/ontology#geochangChangpowon','CAFE','다온 카페']])('anchors %s to the requested place without inventing distance',async(message,anchorEntityId,category,label)=>{const resolver=new PlaceDiscoveryService(regional as any,undefined,undefined,undefined,new RegionConfigService());const result:any=await resolver.discover('gajo',category,message,{});expect(result).toMatchObject({anchorEntityId,entities:[{programLabel:label,distanceMeters:undefined}]});expect(result.anchorLatitude).toBeUndefined();expect(result.anchorLongitude).toBeUndefined()});

  it('contextually anchors area food and bathing discovery without changing exact facility identity',async()=>{const resolver=new PlaceDiscoveryService(regional as any,undefined,undefined,undefined,new RegionConfigService());const food:any=await resolver.discover('gajo','FOOD','가조온천 근처 밥집?',{});expect(food).toMatchObject({anchorEntityId:'https://gajo-wellness.kr/ontology#gajoHotSpringComplex',entities:[{programLabel:'미가추어탕'}]});const bath=await resolver.resolveReference('gajo','가조온천에서 목욕하고 싶어요.','HOT_SPRING_WELLNESS');expect(bath).toMatchObject({entityId:'https://gajo-wellness.kr/ontology#gajoHotSpringComplex',label:'백두산천지온천'});const exact=await resolver.resolveRequestedDestinations('gajo',['백두산천지온천']);expect(exact[0]).toMatchObject({label:'백두산천지온천',resolved:true,source:'RDM'})});

  it('does not expose an entity navigation reference for ambiguous 가조온천 departure',async()=>{const resolver=new PlaceDiscoveryService(regional as any,undefined,undefined,undefined,new RegionConfigService());expect(await resolver.resolveReference('gajo','가조온천으로 출발')).toBeUndefined()});

  it('prefers the canonical review record over search evidence and exposes no unsafe action',async()=>{const nearby={searchByKeyword:jest.fn(async()=>[{id:'suseungdae-search',name:'수승대',category:'TOURISM_NATURE',lat:35.66,lng:127.85,placeUrl:'https://place.map.kakao.com/suseungdae'}])},copilot={ingestSearchCandidate:jest.fn(async()=>({}))},resolver=new PlaceDiscoveryService(regional as any,undefined,nearby as any,copilot as any,new RegionConfigService());const [destination]:any=await resolver.resolveRequestedDestinations('gajo',['수승대'],{latitude:35.7,longitude:128.02});expect(destination).toMatchObject({entityId:'https://gajo-wellness.kr/ontology#suseungdae',source:'RDM',resolved:true,verificationStatus:'UNKNOWN'});expect(destination.actions).toBeUndefined();expect(nearby.searchByKeyword).not.toHaveBeenCalled();expect(copilot.ingestSearchCandidate).not.toHaveBeenCalled()});
