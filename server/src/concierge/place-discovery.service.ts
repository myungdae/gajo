...
export class PlaceDiscoveryService {
  ...
  async resolveRequestedDestinations(regionId:string,labels:string[],context:any={}){
    const operationalRegionId=context.searchRegionId||regionId,dataset=await this.regionalData?.effectiveDataset(regionId),operationalDataset=operationalRegionId!==regionId?await this.regionalData?.effectiveDataset(operationalRegionId):dataset,config=this.regionConfig?.tryGet?.(operationalRegionId)||this.regionConfig?.get(regionId),bounds=config?.bounds;
    const inside=(place:any)=>!bounds||Boolean(place.lat<=bounds.north&&place.lat>=bounds.south&&place.lng<=bounds.east&&place.lng>=bounds.west);
    const result=[] as any[];
    for(const label of labels){
      ...
      const experienceMatch=dataset&&this.exactCanonicalMatch(dataset.records,label),canonicalMatch=experienceMatch||operationalDataset&&this.exactCanonicalMatch(operationalDataset.records,label);
      ...
      let searched:any;
      try{const found=await (this.nearby as any)?.searchByKeyword?.(label,operationalRegionId,this.contextOrigin(context)),matches=found?.filter((place:any)=>inside(place)&&(this.normalize(place.name)===this.normalize(label)||this.normalize(place.name).includes(this.normalize(label))||this.normalize(label).includes(this.normalize(place.name))))||[];searched=matches.length===1?matches[0]:undefined;if(matches.length>1){result.push({label,requestedLabel:label,resolved:false,requested:true,source:'SEARCH',verificationStatus:'UNVERIFIED',ambiguity:{candidates:matches.map((place:any)=>({provider:place.provider||'KAKAO',providerPlaceId:place.providerPlaceId||place.id,label:place.name,address:place.roadAddress||place.address}))}});continue}}catch(error){if(!(error instanceof NearbyServiceError))throw error}
      if(searched){const provider=searched.provider||'KAKAO',providerPlaceId=searched.providerPlaceId||searched.id,destination={entityId:`provider:${String(provider).toLowerCase()}:${providerPlaceId}`,label:searched.name,requestedLabel:label,resolved:true,requested:true,source:'SEARCH',provider,providerPlaceId,address:searched.roadAddress||searched.address,regionId:operationalRegionId,category:searched.category==='OTHER'?'TOURISM_NATURE':searched.category,entityType:searched.category,latitude:searched.lat,longitude:searched.lng,verificationStatus:'PROVIDER_VERIFIED',evidence:{sourceType:'KAKAO_LOCAL',sourceUrl:searched.placeUrl,providerCategory:searched.providerCategoryName,demandSignal:'EXPLICIT_DESTINATION_REQUEST'}};result.push(destination);await this.copilot?.ingestSearchCandidate({regionId:operationalRegionId,displayName:searched.name,category:destination.category,entityType:destination.entityType,address:searched.roadAddress||searched.address,phone:searched.phone,latitude:searched.lat,longitude:searched.lng,evidence:{...destination.evidence,providerPlaceId,discoveredAt:new Date().toISOString()}}).catch(()=>undefined);continue}
