...
describe('NearbyController input policy', () => {
  ...
  it('allows the lodging policy maximum and leaves an omitted radius to automatic expansion',async()=>{await controller.discovery({category:'LODGING',latitude:35.5,longitude:128,radius:10000,regionId:'hapcheon'});await controller.discovery({category:'LODGING',latitude:35.5,longitude:128,regionId:'hapcheon'});expect(nearby.searchProgressively).toHaveBeenNthCalledWith(1,'LODGING',35.5,128,expect.anything(),'hapcheon',10000,'hapcheon',undefined);expect(nearby.searchProgressively).toHaveBeenNthCalledWith(2,'LODGING',35.5,128,expect.anything(),'hapcheon',undefined,'hapcheon',undefined)});
