...
export class NearbyController {
  ...
  @Get('anchors') async anchors(@Query('regionId') regionId?:string,@Query('locale') locale?:string) { if(!regionId?.trim())throw new BadRequestException('regionId가 필요합니다.');return localizeVisitorPayload({results:await this.nearby.representativeAnchors(regionId.trim())},normalizeVisitorLocale(locale)); }
