...
describe('NearbyService', () => {
  ...
  it('returns only verified candidates when lodging remains scarce through 10km',async()=>{jest.spyOn(global,'fetch').mockResolvedValue(response({documents:[{...document('AD5','유효 숙소'),id:'valid',address_name:'경상남도 합천군'},{...document('AD5','타지역 숙소'),id:'outside',y:'36.2',address_name:'경상남도 산청군'}],meta:{is_end:true}}));const result=await new NearbyService(config({KAKAO_REST_API_KEY:'key'})).searchProgressively('LODGING',35.6,128.05,{},'hapcheon');expect(result.radius).toBe(10000);expect(result.results.map(row=>row.id)).toEqual(['valid'])});
  ...
  it('does not count duplicate provider ids toward the validated minimum',async()=>{const fetchSpy=jest.spyOn(global,'fetch').mockImplementation(async(input:any)=>response({documents:Array.from({length:5},()=>({...document('AD5','중복 숙소'),id:'same-stay',address_name:'경상남도 합천군'})),meta:{is_end:true}}));const result=await new NearbyService(config({KAKAO_REST_API_KEY:'key'})).searchProgressively('LODGING',35.6,128.05,{},'hapcheon');expect(result).toMatchObject({radius:10000});expect(result.results).toHaveLength(1);expect(fetchSpy).toHaveBeenCalledTimes(20)});
  it('caps empty lodging searches at four radii, five queries and three pages',async()=>{const fetchSpy=jest.spyOn(global,'fetch').mockResolvedValue(response({documents:[],meta:{is_end:false}}));const result=await new NearbyService(config({KAKAO_REST_API_KEY:'key'})).searchProgressively('LODGING',35.6,128.05,{},'hapcheon');expect(result).toMatchObject({radius:10000,nextRadius:undefined});expect(fetchSpy).toHaveBeenCalledTimes(60)});
