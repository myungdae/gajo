...
describe('NearbyService', () => {
  ...
  it('preserves verified candidates when the time budget expires between radius steps',async()=>{let calls=0;const now=jest.spyOn(Date,'now').mockImplementation(()=>calls>=5?10001:0),fetchSpy=jest.spyOn(global,'fetch').mockImplementation(async()=>{calls++;return response({documents:calls===1?[{...document('AD5','직전 단계 숙소'),id:'previous',address_name:'경상남도 합천군'}]:[],meta:{is_end:true}})});const result=await new NearbyService(config({KAKAO_REST_API_KEY:'key'})).searchProgressively('LODGING',35.6,128.05,{},'hapcheon');expect(result).toMatchObject({coverageStatus:'PARTIAL',providerCalls:5});expect(result.results.map(row=>row.id)).toEqual(['previous']);expect(fetchSpy).toHaveBeenCalledTimes(5);now.mockRestore()});
