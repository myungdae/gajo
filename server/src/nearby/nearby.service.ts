...
export class NearbyService {
  ...
  private async searchWide(category:NearbyCategory,lat:number,lng:number,radius:NearbyRadius,options:NearbySearchOptions,budget:ProviderSearchBudget){
    const [center,...outerRing]=wideSearchCenters(lat,lng,radius as 30000|40000|50000),merged=new Map<string,NearbyPlace>();
    ...
    const query=async(points:{lat:number;lng:number}[])=>{for(let start=0;start<points.length;start+=WIDE_SEARCH_CONCURRENCY){const settled=await Promise.allSettled(points.slice(start,start+WIDE_SEARCH_CONCURRENCY).map(point=>this.search(category,point.lat,point.lng,20000,options,undefined,30,budget,true,true)));for(const result of settled)if(result.status==='fulfilled')merge(result.value);else budget.partialFailure=true;if(budget.exhausted)break}};
    await query([center]);
    if(merged.size<30&&!budget.exhausted)await query(outerRing);
    return[...merged.values()].sort((a,b)=>(a.distanceMeters??Infinity)-(b.distanceMeters??Infinity)).slice(0,30);
  }
