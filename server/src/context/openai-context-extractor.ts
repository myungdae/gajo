...
export class OpenAIContextExtractor implements ContextExtractor {
  ...
  async extract(text: string): Promise<ExtractorResult> {
    ...
    try {
      console.log("[OPENAI_CALL] type=CONTEXT model=" + model);
      ...
      const body:any=await response.json(); console.log("[OPENAI_USAGE] type=CONTEXT model=" + model + " inputTokens=" + (body.usage?.input_tokens ?? 0) + " outputTokens=" + (body.usage?.output_tokens ?? 0)); const outputText=body.output_text || body.output?.flatMap((o:any)=>o.content||[]).find((c:any)=>c.type==='output_text')?.text;
