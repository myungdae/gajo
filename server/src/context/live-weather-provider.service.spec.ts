...
describe('LiveWeatherProviderService', () => {
  ...
  it('honors the shorter per-request timeout and returns a safe fallback',async()=>{const service=new LiveWeatherProviderService(config);jest.spyOn(global,'fetch').mockImplementation(((_url:any,options:any)=>new Promise((_resolve,reject)=>options.signal.addEventListener('abort',()=>reject(new Error('aborted')))))as any);const started=Date.now(),result=await service.getCurrent({regionId:'hapcheon',gpsAllowed:true,latitude:35.5,longitude:128.1,accuracy:0,timeoutMs:100});expect(result).toMatchObject({status:'UNAVAILABLE',source:'UNAVAILABLE'});expect(Date.now()-started).toBeLessThan(500)});
});
