...
describe('RuntimeContextService natural-language hydration', () => {
  it('persists parsed operational fields for the demo sentence', async () => {
    ...
    const result = await service.createContext({ regionId:'gajo', rawMessage: '78세 어머니와 가조에 왔습니다. 어머니는 무릎이 조금 불편합니다. 자동차로 이동하고 오후 5시까지 머물 예정입니다. 지금 상황에 맞게 편안한 일정을 추천해 주세요.' });
  ...
  it('applies a mentioned follow-up patch without resetting preserved context', async () => {
    ...
    const service=new RuntimeContextService(model,traversal,{} as any);
    await service.createContext({regionId:'gajo',rawMessage:'차를 두고 이제 걸어갈게요',isFollowup:true,transportMode:'CAR',stayUntil:'17:00',companions:[{age:78,relationship:'mother',healthConditions:[]}]});
  ...
  it('keeps structured selection authoritative over conflicting initial free text',async()=>{
    ...
    await new RuntimeContextService(model,traversal,{} as any,gateway).createContext({regionId:'gajo',inputMode:'FREE_TEXT',rawMessage:'걸어가도 될 것 같아요',transportMode:'CAR',contextSessionId:'s'});
  ...
  it('merges the exact structured then free-language flow additively',async()=>{
    ...
    const service=new RuntimeContextService(model,traversal,{} as any);
    await service.createContext({regionId:'gajo',inputMode:'STRUCTURED',companions:[{relationship:'parent',healthConditions:[]}],transportMode:'CAR',walkingLevel:'LOW',stayUntil:'17:00',companionConstraints:['shortWalkingDistance'],wellnessGoals:['restAndRecovery'],activityPreferences:['HOT_SPRING','REST_AND_RECOVERY']});
    const first=created[0];
    await service.createContext({regionId:'gajo',inputMode:'FREE_TEXT',isFollowup:true,rawMessage:'엄마가 조금만 움직여도 금방 피곤해하시고 온천은 꼭 하고 싶어요.',companions:first.companions,transportMode:first.transportMode,walkingLevel:first.walkingLevel,stayUntil:first.stayUntil,companionConstraints:first.companionConstraints,wellnessGoals:first.wellnessGoals,activityPreferences:first.activityPreferences});
  ...
  it('aligns explicit LOW walking constraint into ontology-aware expanded conditions',async()=>{
    let created:any;const model={create:jest.fn(async(value:any)=>{created=value;return{toObject:()=>value}})} as any;
    const traversal={expandConditions:()=>({expanded:[],risks:[],evidence:[]}),evaluateRules:()=>[],individualsOfIncludingSubclasses:()=>['operation']} as any;
    await new RuntimeContextService(model,traversal,{} as any).createContext({regionId:'gajo',walkingLevel:'LOW',companionConstraints:['elderlyCompanion','shortWalkingDistance']});
  ...
  it('passes a structured quick-start preset into RuntimeContext without invoking the extractor',async()=>{
    ...
    await new RuntimeContextService(model,traversal,{} as any,gateway).createContext({regionId:'gajo',inputMode:'STRUCTURED',companions:[{relationship:'parent',healthConditions:[]}],walkingLevel:'LOW',companionConstraints:['shortWalkingDistance'],wellnessGoals:['restAndRecovery'],activityPreferences:['REST_AND_RECOVERY']});
    expect(gateway.extract).toHaveBeenCalledWith({regionId:'gajo',text:undefined,sessionId:undefined,followup:undefined});
