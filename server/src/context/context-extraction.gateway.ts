...
export interface GatewayRequest {text?:string;sessionId?:string;followup?:boolean}
...
export class ContextExtractionGateway {
  ...
  async extract(request:GatewayRequest):Promise<GatewayResult>{
    ...
    const session=request.sessionId||'anonymous', key=`${session}:${request.followup?'F':'I'}:${text}`; const cached=this.cache.get(key);
    ...
    const max=Math.max(0,Number(this.config.get('MAX_CONTEXT_LLM_CALLS_PER_SESSION')??3)), count=this.callsBySession.get(session)||0;
    ...
    this.callsBySession.set(session,count+1);this.metrics.totalCalls++;const result=await this.extractor.extract(text);this.cache.set(key,result);
