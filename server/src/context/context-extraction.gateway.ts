...
export class ContextExtractionGateway {
  ...
  async extract(request:GatewayRequest):Promise<GatewayResult>{
    ...
    if(count>=max){this.metrics.limitSkips++;this.metrics.fallbackCalls++;void this.aiUsageLedger?.record({component:'CONTEXT',eventType:'BLOCKED',reason:'SESSION_LIMIT'});return{decision:'FALLBACK',invocationReason:request.followup?'FREE_TEXT_FOLLOWUP':'FREE_TEXT_INITIAL',limitReached:true,result:{status:'DISABLED',provider:'gateway',latencyMs:0,errorCode:'SESSION_LIMIT'}}}
    this.callsBySession.set(regionalSession,count+1);this.metrics.totalCalls++;void this.aiUsageLedger?.record({component:'CONTEXT',eventType:'CALL'});const result=await this.extractor.extract(text);this.cache.set(key,result);void this.aiUsageLedger?.record({component:'CONTEXT',eventType:result.status==='SUCCESS'?'SUCCESS':'ERROR',modelName:result.model,inputTokens:result.usage?.inputTokens||0,outputTokens:result.usage?.outputTokens||0,latencyMs:result.latencyMs,errorCode:result.errorCode});
