...
export class GuideService{
 ...
 private resolve(question:string,previousIntent?:string){if(/구글도.*(?:추천|ai)|chatgpt.*위치.*똑같/i.test(question)||previousIntent==='MAP_DIFFERENCE'&&/그래도|추천|ai|똑같/.test(question))return GUIDE_KNOWLEDGE.find(x=>x.intent==='MAP_OBJECTION');return GUIDE_KNOWLEDGE.map(entry=>({entry,score:entry.patterns.reduce((sum,p)=>sum+(p.test(question)?1:0),0)+(entry.intent===previousIntent&&/그래도|그런데|그럼/.test(question)?1:0)})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score)[0]?.entry}
 ...
 private present(knowledge:GuideKnowledge,audience:GuideAudience){const perspective=audience==='BUSINESS'?'업체 입장에서 보면, 정확한 정보와 적절한 행동 연결이 핵심입니다.':audience==='PUBLIC_SECTOR'?'공공 관점에서는 신뢰할 수 있는 지역 데이터 관리와 지속성이 핵심입니다.':undefined;return{status:'ANSWERED',intent:knowledge.intent,audience,answer:[knowledge.shortAnswer,perspective].filter(Boolean).join(' '),supportingConcepts:knowledge.supportingConcepts,relatedQuestions:knowledge.relatedQuestions,readOnly:true}}
}
