...
export class GuideService{
 ...
 approvedExplanation(input:{question?:string;audience?:GuideAudience;previousIntent?:string}){
  const question=String(input.question||'').trim().slice(0,500),knowledge=this.resolve(question,input.previousIntent);if(!question||!knowledge)return undefined;return this.present(knowledge,this.audience(question,input.audience))
 }
 answer(input:{question?:string;audience?:GuideAudience;previousIntent?:string},clientId='anonymous'){
  this.limit(clientId);const question=String(input.question||'').trim().slice(0,500);if(!question)return{status:'INVALID',message:'질문을 입력해 주세요.'};const approved=this.approvedExplanation(input);if(!approved)return{status:'REVIEW_CANDIDATE',candidate:{type:'NEW_GUIDE_QUESTION',questionStored:false,requiresHumanApproval:true},answer:'아직 승인된 안내 지식에서 정확한 답을 찾지 못했습니다. 운영 정보는 추측하지 않고 검토가 필요한 질문으로 구분할게요.',audience:this.audience(question,input.audience)};return approved
 }
