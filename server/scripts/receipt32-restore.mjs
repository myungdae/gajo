...
try{await mongoose.connect(uri,{autoIndex:false,autoCreate:false});const collection=mongoose.connection.db?.collection(manifest.collection);if(!collection)throw new Error('Configured collection is unavailable');const result=await runReceipt32Restore({collection,preImage,manifest,actorId,reason,apply});process.stdout.write(`${JSON.stringify(result,null,2)}\n`)}catch(error){const safe=String(error?.message||'unknown error').replace(/mongodb(?:\+srv)?:\/\/\S+/gi,'[REDACTED_MONGODB_URI]');process.stderr.write(`Receipt 32 restore failed: ${safe}\n`);process.exitCode=1}finally{await mongoose.disconnect()}
