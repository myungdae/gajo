...
export type JourneyGoal='FOOD'|'CAFE'|'NEXT_PLACE'|'EVENT_TODAY';
export type JourneyPreferences={goal?:JourneyGoal;companion?:'ALONE'|'COUPLE'|'CHILDREN'|'PARENTS'|'FRIENDS';duration?:'ONE_HOUR'|'TWO_THREE_HOURS'|'HALF_DAY'|'DAY';transport?:'CAR'|'WALK'|'PUBLIC_TRANSPORT';walking?:'ANY'|'LOW'|'EASY';replacePlace?:boolean};
export const JOURNEY_COPY={ko:{question:'무엇을 하고 싶으세요?',auto:'현재 위치와 날씨는 확인된 경우에만 자동으로 반영합니다.',conditions:'여행 조건 더하기',create:'내 여정 만들기',direct:'직접 말하거나 글로 요청하기',startQuestion:'이 여정으로 출발할까요?',start:'이대로 시작',adjust:'조금 바꾸기',other:'다른 요청하기',speak:'말하기',type:'글로 입력하기',unknown:'확인하지 못한 정보는 판단에서 제외합니다.'},en:{question:'What would you like to do?',auto:'Your location and weather are used automatically only when verified.',conditions:'Add travel preferences',create:'Create My Journey',direct:'Speak or type a request',startQuestion:'Shall we start this journey?',start:'Start This Journey',adjust:'Make a Small Change',other:'Make Another Request',speak:'Speak',type:'Type',unknown:'Information we could not verify is left out.'}} as const;
export const JOURNEY_OPTIONS={goal:[['FOOD','밥 먹기','Eat'],['CAFE','카페에서 쉬기','Rest at a Café'],['NEXT_PLACE','다음 장소 찾기','Find the Next Place'],['EVENT_TODAY','오늘 행사','Events Today']],companion:[['ALONE','혼자','Solo'],['COUPLE','연인','Partner'],['CHILDREN','아이와','With Children'],['PARENTS','부모님과','With Parents'],['FRIENDS','친구와','With Friends']],duration:[['ONE_HOUR','1시간','1 Hour'],['TWO_THREE_HOURS','2~3시간','2–3 Hours'],['HALF_DAY','반나절','Half Day'],['DAY','하루','Full Day']],transport:[['CAR','자동차','Car'],['WALK','도보','Walking'],['PUBLIC_TRANSPORT','대중교통','Public Transit']],walking:[['ANY','상관없어요','No Preference'],['LOW','조금만 걷기','Minimal Walking'],['EASY','쉬엄쉬엄','Easy Pace']]} as const;
export function journeyRequest(p:JourneyPreferences,language:'ko'|'en'):{text:string;context:CreateContextInput;planned:PlannedContext}{
  ...
  const values=[label('goal',p.goal),label('companion',p.companion),label('duration',p.duration),label('transport',p.transport),label('walking',p.walking),p.replacePlace?(language==='ko'?'장소 하나 바꾸기':'replace one place'):undefined].filter(Boolean);
...
export function startRuntimeJourney(regionId:string,itinerary:any,storage:Pick<Storage,'getItem'|'setItem'>=localStorage){
  const current=loadTripSession(storage,regionId),steps=Array.isArray(itinerary?.steps)?itinerary.steps:[];
