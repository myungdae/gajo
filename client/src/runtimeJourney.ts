...
export function journeyPreferences(context: CreateContextInput, now = new Date()): JourneyPreferences {
  const relationship = context.companions?.[0]?.relationship;
  const clock=new Date(now.getTime()+9*3600000),deadline=context.stayUntil?.match(/^(\d{2}):(\d{2})$/),remaining=deadline?Number(deadline[1])*60+Number(deadline[2])-clock.getUTCHours()*60-clock.getUTCMinutes():undefined;
  return {
    duration: remaining&&remaining>0?(remaining<=60?'ONE_HOUR':remaining<=180?'TWO_THREE_HOURS':remaining<=300?'HALF_DAY':'DAY'):undefined,
    companion: context.companions?.length === 0 ? 'ALONE' : ({ parent:'PARENTS', mother:'PARENTS', father:'PARENTS', child:'CHILDREN', partner:'COUPLE', spouse:'COUPLE', friend:'FRIENDS' } as const)[relationship as 'parent'],
    transport: ['CAR','WALK','PUBLIC_TRANSPORT'].includes(context.transportMode || '') ? context.transportMode as JourneyPreferences['transport'] : undefined,
    walking: context.walkingLevel === 'LOW' ? 'LOW' : context.walkingLevel ? 'ANY' : undefined,
    goal: ['FOOD','CAFE','ACCOMMODATION','NEXT_PLACE','EVENT_TODAY'].find(goal => context.activityPreferences?.includes(goal)) as JourneyGoal | undefined,
  };
}
export const JOURNEY_COPY={ko:{question:'지금 무엇을 하고 싶으세요?',auto:'현재 위치와 날씨는 확인된 경우에만 자동으로 반영합니다.',conditions:'내 여행에 맞게 더 알려주세요',durationQuestion:'오늘 여행할 시간이 얼마나 있으신가요?',create:'내 여정 만들기',direct:'원하는 것을 말하거나 글로 알려주세요',startQuestion:'다음 행동',start:'이 일정으로',adjust:'조금 바꾸기',other:'다른 요청하기',speak:'말하기',type:'글로 입력하기',unknown:'확인하지 못한 정보는 판단에서 제외합니다.'},en:{question:'What would you like to do?',auto:'Your location and weather are used automatically only when verified.',conditions:'Add travel preferences',durationQuestion:'How much time do you have for today\'s trip?',create:'Create My Journey',direct:'Speak or type a request',startQuestion:'Next action',start:'Use this itinerary',adjust:'Make a Small Change',other:'Make Another Request',speak:'Speak',type:'Type',unknown:'Information we could not verify is left out.'}} as const;
...
export function journeyRequest(p:JourneyPreferences,language:'ko'|'en', now = new Date()):{text:string;context:CreateContextInput;planned:PlannedContext}{
  const label=(key:keyof typeof JOURNEY_OPTIONS,value?:string)=>JOURNEY_OPTIONS[key].find(row=>row[0]===value)?.[language==='ko'?1:2];
  const companion=p.companion==='ALONE'?[]:p.companion?[{relationship:({COUPLE:'partner',CHILDREN:'child',PARENTS:'parent',FRIENDS:'friend'} as any)[p.companion],healthConditions:[]}]:undefined;
  ...
  const minutes = p.duration && ({ONE_HOUR:60,TWO_THREE_HOURS:150,HALF_DAY:240,DAY:480})[p.duration];
  const seoul = new Date(now.getTime() + 9 * 3600000), end = minutes ? Math.min(1439, seoul.getUTCHours()*60+seoul.getUTCMinutes()+minutes) : undefined;
  const stayUntil = end === undefined ? undefined : `${String(Math.floor(end/60)).padStart(2,'0')}:${String(end%60).padStart(2,'0')}`;
  const context:CreateContextInput={inputMode:'STRUCTURED',stayUntil,activityPreferences:p.goal?[p.goal]:undefined,companions:companion,companionConstraints:constraints,walkingLevel,transportMode:p.transport};
  ...
  return {text:language==='ko'?`${values.join(', ')||'현재 상황'}에 맞는 순서 있는 지역여정을 만들어 주세요.`:`Create an ordered regional journey for ${values.join(', ')||'my current context'}.`,context,planned:Object.fromEntries(Object.entries({stayUntil,duration:p.duration==='DAY'?'DAY':undefined,companions:companion,mobilityConstraints:constraints,walkingLevel,transportMode:p.transport,interests:p.goal?[p.goal]:undefined}).filter(([,value])=>value!==undefined))};
}
