...
test('Kakao sharing exists only on the landing and remains optional behind readiness state',()=>{const landingShare=readFileSync(new URL('./components/RegionalLandingShare.tsx',import.meta.url),'utf8'),layout=readFileSync(new URL('./components/Layout.tsx',import.meta.url),'utf8');assert.match(landingShare,/VITE_KAKAO_JAVASCRIPT_KEY/);assert.match(landingShare,/kakaoKey&&<button/);assert.match(landingShare,/disabled=\{kakaoState!==['"]READY['"]\}/);assert.match(landingShare,/Kakao\.Share\.sendDefault/);assert.match(landingShare,/script\.onerror/);assert.match(landingShare,/initializeKakaoShare/);assert.match(landingShare,/entry-qr/);assert.doesNotMatch(layout,/CompanionShare|동행자 초대|공유 모달/)});
test('the Hapcheon start action prevents the landing from reappearing inside the active trip',()=>{const landing=readFileSync(new URL('./pages/HapcheonLandingPage.tsx',import.meta.url),'utf8'),app=readFileSync(new URL('./App.tsx',import.meta.url),'utf8'),layout=readFileSync(new URL('./components/Layout.tsx',import.meta.url),'utf8');assert.match(landing,/sessionStorage\.setItem\(['"]hapcheon-landing-complete['"]/);assert.match(app,/sessionStorage\.getItem\(['"]hapcheon-landing-complete['"]\)===['"]1['"]/);assert.match(layout,/\/hapcheon\?start=ai/)});
