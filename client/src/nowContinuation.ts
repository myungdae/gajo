import { REGION_INTEREST_OPTIONS } from './regionConfig.ts';
...
export function buildNowContinuation(planned?:PlannedContext){
  ...
  const interests=(planned.interests||[]).map(id=>REGION_INTEREST_OPTIONS.find(option=>option.id===id)?.label).filter(Boolean) as string[];
