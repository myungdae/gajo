...
import { buildContextSummary,withRequestedDestinations } from './contextSummary.ts';
...
test('requested destinations appear first and preserve visitor terminology',()=>{const rows=withRequestedDestinations(buildContextSummary({companions:[{relationship:'spouse',healthConditions:[]}],walkingLevel:'MODERATE',transportMode:'CAR',activityPreferences:['HOT_SPRING','REST_AND_RECOVERY']}),[{label:'백두산천지온천',requestedLabel:'가조온천'},{label:'수승대',requestedLabel:'수승대'}]);assert.deepEqual(rows.map(({label,value})=>({label,value})),[{label:'꼭 가고 싶은 곳',value:'가조온천 · 수승대'},{label:'동반',value:'배우자'},{label:'보행',value:'보통 수준 보행'},{label:'이동',value:'자동차'},{label:'관심',value:'온천 · 편안한 휴식'}])});
