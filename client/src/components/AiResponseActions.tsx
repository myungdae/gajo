...
export default function AiResponseActions({ rawMessage, result, turnId, excludedEntityIds, onExcludedEntityIdsChange, onReplaceAlternative }: { rawMessage: string; result: ConciergeChatResponse; turnId: string; excludedEntityIds?: string[]; onExcludedEntityIdsChange?: (ids: string[]) => void; onReplaceAlternative?: (entity: any) => void }) {
  ...
  const effectiveExcluded = excludedEntityIds ?? excluded;
  const model = buildAiResponseActionModel({ rawMessage, result, hasCurrentItinerary: Boolean((session.itinerary as any)?.steps?.length), excludedEntityIds: effectiveExcluded });
