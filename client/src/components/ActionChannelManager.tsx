...
export default function ActionChannelManager({regionId,placeKey,token}:{regionId:string;placeKey:string;token:string}) {
  ...
  return <section className="action-channel-manager" aria-label="검증된 행동 채널 관리">
    ...
    </details>{templates.map((item,i)=><button type="button" key={i} onClick={()=>{setSelected(null);setForm({...empty(),kind:item.kind,labelKo:item.labelKo,labelEn:item.labelEn,target:item.target,sourceUrl:item.sourceUrl})}}>검토 자료: {item.labelKo}</button>)}
