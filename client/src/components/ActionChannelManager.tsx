...
export default function ActionChannelManager({regionId,placeKey,token}:{regionId:string;placeKey:string;token:string}) {
  const [rows,setRows]=useState<any[]>([]),[form,setForm]=useState(empty),[selected,setSelected]=useState<any>(null),[notice,setNotice]=useState(''),[busy,setBusy]=useState(false),[confirmed,setConfirmed]=useState(false),[templates,setTemplates]=useState<any[]>([]);
  ...
  useEffect(()=>{let active=true;setRows([]);setSelected(null);setForm(empty());if(token)api.get(`/action-channels/admin?${query}`,{headers}).then(({data})=>{if(active)setRows(data)}).catch(()=>{if(active)setNotice('채널 조회 권한을 확인해 주세요.')});return()=>{active=false}},[query,token]);
  ...
  }catch{setNotice('저장하지 못했습니다. 지역 권한·URL·검수 기한을 확인하거나 다시 조회하세요.')}finally{setBusy(false)}};
  const edit=(row:any)=>{setSelected(row);setConfirmed(false);setForm({kind:row.kind,labelKo:row.labelKo,labelEn:row.labelEn,target:row.target,sourceUrl:row.sourceUrl,reviewDueAt:row.reviewDueAt.slice(0,10)})};
  return <section className="action-channel-manager" aria-label="검증된 행동 채널 관리">
    ...
    <label>채널 검토 자료 불러오기 (JSON, 자동 등록·공개 없음)<input type="file" accept="application/json,.json" onChange={async e=>{const file=e.target.files?.[0];if(!file)return;try{if(file.size>50000)throw new Error();const pack=JSON.parse(await file.text());if(pack.regionId!==regionId||pack.canonicalEntityId!==placeKey||!Array.isArray(pack.channels)||pack.channels.length>20||pack.channels.some((c:any)=>!c||!Object.keys(CHANNEL_LABELS.ko).includes(c.kind)||[c.labelKo,c.labelEn,c.target,c.sourceUrl].some(v=>typeof v!=='string'||v.length>1000)))throw new Error();setTemplates(pack.channels);setNotice('자료의 채널을 선택한 뒤 근거를 확인하고 초안으로 등록하세요.')}catch{setNotice('선택한 지역·장소와 일치하는 검토 자료가 아닙니다.')}}}/></label>
    {templates.map((item,i)=><button type="button" key={i} onClick={()=>{setSelected(null);setForm({...empty(),kind:item.kind,labelKo:item.labelKo,labelEn:item.labelEn,target:item.target,sourceUrl:item.sourceUrl})}}>검토 자료: {item.labelKo}</button>)}
    <form onSubmit={e=>{e.preventDefault();void run(selected?'EDIT':'CREATE')}}>
      <label>채널 종류<select value={form.kind} onChange={e=>{const kind=e.target.value as ChannelKind;setForm({...form,kind,labelKo:CHANNEL_LABELS.ko[kind],labelEn:CHANNEL_LABELS.en[kind]})}}>{Object.entries(CHANNEL_LABELS.ko).map(([kind,label])=><option key={kind} value={kind}>{label}</option>)}</select></label>
      {([['labelKo','한국어 표시명'],['labelEn','영어 표시명'],['target','정확한 업체별 URL 또는 전화번호'],['sourceUrl','공식 근거 URL'],['reviewDueAt','재검수 기한']] as const).map(([key,label])=><label key={key}>{label}<input required type={key==='reviewDueAt'?'date':key==='sourceUrl'?'url':'text'} maxLength={key.startsWith('label')?80:1000} value={form[key]} onChange={e=>setForm({...form,[key]:e.target.value})}/></label>)}
      <button className="btn btn-primary" disabled={busy}>{selected?'수정 저장 · 공개 해제':'초안 등록'}</button><button type="button" className="btn btn-outline" onClick={()=>{setSelected(null);setForm(empty())}}>새 채널 작성</button>
    </form>
    ...
    {rows.map(row=><article key={row.channelId}><h4>{row.labelKo} / {row.labelEn}</h4><p>{new Date(row.reviewDueAt)<=new Date()?'REVIEW_REQUIRED':row.verificationStatus} · {row.published&&row.verificationStatus==='VERIFIED'&&new Date(row.reviewDueAt)>new Date()?'공개':'비공개'} · {new Date(row.reviewDueAt)<=new Date()?'재검수 필요':'검수 기한'}: {row.reviewDueAt.slice(0,10)}</p><p className="channel-target">{row.target}</p>
