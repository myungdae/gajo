import VerifiedChannelActions from "./VerifiedChannelActions";
...
import { phoneActionLabel, publicEntityActions } from '../entityActionPolicy';
...
export default function EntityActions({entity,hideDetail=false,navigationLabel,onNavigate,showItineraryAdd=true}:{entity:any;hideDetail?:boolean;navigationLabel?:string;onNavigate?:(provider:NavigationProvider)=>void;showItineraryAdd?:boolean}){
  const region=useRegion(),session=ensureTripSession(region.id),actions=publicEntityActions(entity.actions||entity.literalProps?.actions),normalized={...entity,actions},resolvedNavigationLabel=navigationLabel||navigationActionLabel(normalized);
  const [channelKinds,setChannelKinds]=useState<string[]>([]);
  ...
  useEffect(()=>setChannelKinds([]),[region.id,id]);
  ...
  return <><div className="entity-actions">{!hideDetail&&actions.detail?.url&&<a className="btn btn-text" href={actions.detail.url} target="_blank" rel="noopener noreferrer" onClick={()=>event('ENTITY_DETAIL_OPENED','DETAIL')}>상세보기</a>}{id&&<VerifiedChannelActions key={`${region.id}:${id}`} regionId={region.id} placeKey={id} onKinds={setChannelKinds}/>}{!channelKinds.includes('PHONE')&&actions.call?.phone&&<a className="btn btn-text" href={`tel:${actions.call.phone.replace(/[^0-9+]/g,'')}`} onClick={()=>event('PHONE_HANDOFF','CALL')}>{phoneActionLabel(entity,actions)}</a>}{!channelKinds.includes('OFFICIAL_WEBSITE')&&actions.website?.url&&<a className="btn btn-text" href={actions.website.url} target="_blank" rel="noopener noreferrer" onClick={()=>event('WEBSITE_HANDOFF','WEBSITE')}>홈페이지</a>}{actions.navigate&&<details><summary className="btn btn-text">{resolvedNavigationLabel}</summary>{providers}{actions.navigate.evidenceMode==='OFFICIAL_PREVIEW'&&<p className="action-helper">공식 데이터에 등록된 위치를 기준으로 길찾기를 엽니다.</p>}</details>}{showItineraryAdd&&id&&<button type="button" className="btn btn-text" onClick={add}>내 여행에 담기</button>}</div>{result&&<ItineraryAddContinuation entity={normalized} result={result} onStart={actions.navigate?()=>setStartChoices(true):undefined} onReset={()=>{setResult(null);setStartChoices(false)}}/>}{startChoices&&<div className="entity-start-providers" role="group" aria-label={`${name}으로 출발할 내비 선택`}>{providers}</div>}</>;
}
