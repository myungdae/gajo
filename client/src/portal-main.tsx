...
const regions = [
  { id: 'gajo', name: '거창 / 가조', shortName: '거창', tag: '산과 온천, 자연 속 쉼', use: '처음 온 가족 여행부터 지금 필요한 다음 장소까지', preview: '현재 운영 중인 지역 컨시어지로 연결됩니다.' },
...
function Portal() {
  return <div className="portal-shell">
    <header className="portal-nav"><a className="brand" href="/portal.html" aria-label="Regional AI Concierge 포털 홈"><span>R</span>REGIONAL AI CONCIERGE</a><button onClick={jumpToRegions}>지역 선택</button></header>
    <main>
      <section className="hero" aria-labelledby="hero-title"><div className="hero-copy"><p className="eyebrow">REGIONAL AI CONCIERGE</p><h1 id="hero-title">대한민국 어디서든,<br/>그 지역을 가장 잘 아는 AI와 여행하세요</h1><p className="hero-lead">메뉴를 배우지 마세요.<br/>그냥 지금 상황을 말씀하세요.</p><div className="hero-actions"><button className="primary" onClick={jumpToRegions}>지역 AI 컨시어지 시작하기</button><a href="#difference">어떻게 다른가요?</a></div><small>확인 가능한 위치·시간·날씨와 방문자가 알려준 여행 상황을 바탕으로 다음 선택을 돕습니다.</small></div><div className="hero-visual" aria-label="계획부터 행동까지 이어지는 여행의 흐름">{[['PLAN','부모님과 옥천 하루 여행하고 싶어요.'],['NOW','지금 오후 4시고 여기까지 봤어요.'],['RE-PLAN','비가 오고 어머니가 좀 힘들어하세요.'],['ACTION','그럼 가까운 곳부터 갈게. 길찾기 해줘.']].map(([step,example],index)=><div className="journey-step" key={step}><span>{step}</span><p>“{example}”</p>{index<3&&<i aria-hidden="true">↓</i>}</div>)}</div></section>
      ...
      <section className="section audience-section"><div className="section-heading"><p className="eyebrow">FOUR WAYS IN</p><h2>지역을 찾는 사람부터<br/>지역을 운영하는 사람까지</h2></div><div className="audience-grid">{[['여행객','우리 지역 AI 만나기'],['지역 업체','우리 가게를 지역 AI와 연결하기'],['지자체·관광기관','우리 지역 Regional Concierge 도입하기'],['Regional Manager / 지역 운영자','지역정보 운영에 참여하기']].map(([title,cta])=><article key={title}><small>{title}</small><h3>{cta}</h3><button onClick={()=>trackPortal('portal_audience_selected',{audience:title})}>Phase 1 안내 보기 →</button></article>)}</div></section>
    ...
    <footer><div><strong>REGIONAL AI CONCIERGE</strong><p>발견하고, 이해하고, 지역을 선택하는 입구.</p></div><button className="primary" onClick={jumpToRegions}>지역 선택하기</button></footer>
  </div>;
}
