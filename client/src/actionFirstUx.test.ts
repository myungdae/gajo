...
test("retired default prompts and multi-field confirmation are not rendered",()=>{
  for(const copy of ["다른 조건도 말씀해 주세요","비가 와 / 배고파 / 카페 가고 싶어","요청 확인 필요","종류 확인 필요","검색 기준 지역 확인"])assert.doesNotMatch(page,new RegExp(copy));
...
test("cancel closes manual entry without changing trip or region",()=>{
  assert.match(page,/const dismissVoice/);
