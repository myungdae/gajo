...
test("NOW starts with one common action hub and secondary manual controls",()=>{
  ...
  for(const label of ["VOICE_COPY","직접 입력하기"])assert.match(page,new RegExp(label));
...
test("retired default prompts and multi-field confirmation are not rendered",()=>{
  ...
  assert.match(confirmation,/<textarea/);assert.match(confirmation,/onChange\(event.target.value\)/);assert.match(confirmation,/disabled=\{sending\|\|!text.trim\(\)\}/);
});
