...
const actionRules: Array<[RegExp, string]> = [
  [/일정에?\s*(담아|넣어|추가해?|잡아)/, "일정에 담기"], [/일정에서?\s*(빼|삭제)/, "일정에서 삭제"],
...
const cleanPlace = (text: string) => text
  ...
  .replace(/찾아\s*줘|찾아줘|찾아|추천해?\s*줘|추천|일정에?\s*(담아|넣어|추가해?|잡아)\s*줘?|일정에서?\s*(빼|삭제)\s*줘?|예약해?\s*줘?|전화해?\s*줘?|길\s*안내(?:를)?\s*(시작해?\s*줘?|해\s*줘)?|몇\s*시까지\s*해|몇\s*시|지금|추워|더워|날씨|정보를?|알려\s*줘|볼까|할까/g, " ")
