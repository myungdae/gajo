...
test("home distinguishes an active itinerary, saved context and a first journey",()=>{
  ...
  assert.match(home,/<RuntimeJourneyEntry loading/);
  assert.ok(home.indexOf('<RuntimeJourneyEntry')<home.indexOf('<TripContinuity'));
  assert.doesNotMatch(home,/hasActiveTrip|hasTripContext/);
  assert.match(continuity,/home-trip-secondary/);
  assert.doesNotMatch(continuity,/현재 상황으로 여정 만들기/);
  ...
  assert.ok(home.indexOf('className={`spotlight-card')<home.indexOf('<TripContinuity'), 'home welcome must precede saved-trip continuity');
});
...
test("replanning is structured and free voice or text remains explicitly requested",()=>{
  ...
  assert.match(result,/setAdjusting\(false\);setCategory\(null\);onText\(\)/);
...
test("other request and adjustment controls are mutually exclusive beside results",()=>{
  ...
  assert.match(actions,/setAdjusting\(false\);setCategory\(null\);onText\(\)/);
