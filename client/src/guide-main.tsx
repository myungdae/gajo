...
type Message = { role: 'user' | 'guide'; text: string; response?: GuideResponse };
...
function GuideApp() {
  ...
  const send = async (value = input) => {
    ...
    setMessages((current) => [...current, { role: 'user', text: question }]); setInput(''); setBusy(true);
    try { const response = await askGuide(question, last?.intent, audience); setMessages((current) => [...current, { role: 'guide', text: response.answer, response }]); }
    catch (error: any) { setMessages((current) => [...current, { role: 'guide', text: error.message || '잠시 후 다시 시도해 주세요.' }]); }
    finally { setBusy(false); }
  };
  const home = () => { landingRef.current?.focus({ preventScroll: true }); landingRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }); };
  const restart = () => { if (window.confirm('이전 질문과 답변을 지우고 새 대화를 시작할까요?')) { setMessages([]); setInput(''); home(); } };
  return <main className="guide-shell">
    ...
    {messages.length > 0 && <section className="guide-conversation" aria-live="polite" aria-label="이전 질문과 답변">{messages.map((message, index) => <article key={index} className={`guide-message ${message.role}`}><small>{message.role === 'user' ? '질문' : '가이드'}</small><p>{message.text}</p>{message.response?.relatedQuestions?.length ? <div className="guide-related">{message.response.relatedQuestions.slice(0, 2).map((question) => <button key={question} onClick={() => send(question)}>{question}</button>)}</div> : null}</article>)}</section>}
