...
test("discovery context retains anchor category primary result exclusions and source turn",()=>{for(const field of ["anchor:","targetCategory:","currentResult:","shownEntityIds:","sourceTurnId:"])assert.ok(page.includes(field));assert.match(api,/distanceInfo\?\s*:\s*\{[\s\S]*?status\s*:\s*["']RESOLVED["']\s*\|\s*["']NEEDS_CLARIFICATION["']/)});
