...
export interface TripSession {
  ...
  runtimeContext?: any;
  locationContext?: { now?:TripLocation; planStart?:TripLocation; tripBaseline?:TripLocation; pendingReplan?:{createdAt:string;from?:TripLocation;to:TripLocation;distanceMeters?:number;impact:"REVIEW_REMAINING_ROUTE";itineraryPreserved:true;fingerprint?:string}; lastReplanSuggestion?:{fingerprint:string;suggestedAt:string} };
  itinerary?: unknown;
  savedPlaces?: any[];
  execution?: {
...
export function locationReplanSuggestion(session:TripSession,prior:TripLocation|undefined,next:TripLocation,now=Date.now()){if(prior?.status!=="CONFIRMED")return undefined;const steps=(session.itinerary as any)?.steps||[],status=session.execution?.statusByEntityId||{},target=steps.find((step:any)=>!['COMPLETED','SKIPPED'].includes(status[step.entityId||step.programUri]||step.status)),destination=target&&{status:"CONFIRMED"as const,source:"TRIP_CONTEXT"as const,latitude:target.latitude??target.actions?.navigate?.latitude,longitude:target.longitude??target.actions?.navigate?.longitude,observedAt:next.observedAt};if(!Number.isFinite(destination?.latitude)||!Number.isFinite(destination?.longitude))return undefined;const before=locationDistance(prior,destination),after=locationDistance(next,destination),moved=locationDistance(prior,next);if(before===undefined||after===undefined||moved===undefined||after-before<Math.max(2000,before*.3))return undefined;const fingerprint=`${target.entityId||target.programUri||target.itemId}:${Math.round(next.latitude!*100)/100}:${Math.round(next.longitude!*100)/100}`,last=session.locationContext?.lastReplanSuggestion;if(last?.fingerprint===fingerprint&&now-Date.parse(last.suggestedAt)<30*60*1000)return undefined;return{createdAt:new Date(now).toISOString(),from:prior,to:next,distanceMeters:moved,impact:"REVIEW_REMAINING_ROUTE"as const,itineraryPreserved:true as const,fingerprint}}
export function confirmTripLocation(regionId:string,mode:"NOW"|"PLAN",location:TripLocation,storage:Pick<Storage,"getItem"|"setItem">=localStorage){const current=loadTripSession(storage,regionId);if(!current)return undefined;const confirmed={...location,status:"CONFIRMED" as const,confirmedAt:new Date().toISOString()},prior=mode==="NOW"?current.locationContext?.now:current.locationContext?.planStart,suggestion=mode==="NOW"?locationReplanSuggestion(current,prior,confirmed):undefined,pendingReplan=suggestion||current.locationContext?.pendingReplan,lastReplanSuggestion=suggestion?{fingerprint:suggestion.fingerprint!,suggestedAt:suggestion.createdAt}:current.locationContext?.lastReplanSuggestion;return saveTripSession({...current,locationContext:{...current.locationContext,...(mode==="NOW"?{now:confirmed}:{planStart:confirmed}),...(!current.locationContext?.tripBaseline?{tripBaseline:confirmed}:{}),...(pendingReplan?{pendingReplan}:{}),...(lastReplanSuggestion?{lastReplanSuggestion}:{})}},storage)}
