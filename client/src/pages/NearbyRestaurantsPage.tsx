...
export default function NearbyRestaurantsPage() {
  ...
  const navigateWith=(provider:NavigationProvider)=>{const destination=navigationPlace&&navigationDestination(navigationPlace);if(!destination)return;launchNavigation(navigationTarget(provider,destination),{mobile:isMobileNavigation(navigator.userAgent)});setNavigationPlace(null)};
  ...
  return <div className="nearby-discovery">
    ...
    {!loading && !error && origin && <>
      ...
      {selected && <section className="card nearby-detail"><div className="nearby-title-row"><div><small>{selected.categoryLabel}</small><h2>{selected.name}</h2></div>{selected.canonicalEntityUri && <span className="badge">등록 장소 연결</span>}</div>
        ...
        <div className="nearby-actions"><a className="btn btn-primary" href={mapLink || selected.placeUrl} target="_blank" rel="noreferrer">지도에서 보기</a>{navigationDestination(selected)&&<button className="btn btn-outline" onClick={()=>setNavigationPlace(selected)}>길찾기</button>}<button className="btn btn-outline" onClick={() => setNotice('일정 변경은 확인 후 반영됩니다. 이 장소를 일정에 넣으려면 현재 일정 화면에서 변경을 요청해 주세요.')}>일정에 담기</button></div>
      </section>}
