import VerifiedChannelActions from "../components/VerifiedChannelActions";
import { visitorTrack,rememberPlace,analyticsPlaceKey } from '../visitorAnalytics';
import { liveRegionName } from '../liveStatusPresentation';
...
export default function NearbyRestaurantsPage() {
  ...
  const analyticsSearch=useRef<{searchId:string;resultSetId:string}|null>(null);
  ...
  useEffect(() => {
    ...
    const analyticsContext={searchId:crypto.randomUUID(),resultSetId:crypto.randomUUID()};analyticsSearch.current=analyticsContext;
    visitorTrack('NEARBY_SEARCH_SUBMITTED',region.id,tripSession.anonymousTripId,{searchId:analyticsContext.searchId});
    ...
    fetchNearbyDiscovery(category, origin[0], origin[1], { radius:requestedRadius,useDistance: distanceTrusted, transportMode: 'car', weather, regionId: region.id,experienceRegionId:region.id,searchRegionId,coordinateSearch:!anchor,regionMembership:operationalLocation?.regionMembership,signal:controller.signal })
      .then(response => {if(current){const results=anchor?response.results.filter(place=>place.canonicalEntityUri!==anchor.entityId&&`provider:kakao:${place.providerPlaceId||place.id}`!==anchor.entityId):response.results;results.forEach(place=>rememberPlace(region.id,place,analyticsContext));setPlaces(results);setDistanceBands(response.distanceBands||[]);setSearchedAt(response.searchedAt);setRadius(response.radius);setNearRadius(response.initialRadius);setNextRadius(response.nextRadius);setExpanded(response.expanded);setCoverageStatus(response.coverageStatus)}})
  ...
  useEffect(()=>{if(searchedAt&&analyticsSearch.current)visitorTrack('SEARCH_RESULTS_SHOWN',region.id,tripSession.anonymousTripId,{...analyticsSearch.current,resultCount:places.length});},[searchedAt,places]);
  const choose = async (place: NearbyPlace) => { track('PLACE_DETAIL_OPENED',tripSession.id,{category:place.category,entityId:analyticsPlaceKey(place)});setSelected(place); setNotice(null); const links = await fetchNavigationLinks(place.lat, place.lng, place.name); setMapLink(links.kakaoMapWeb); };
  const navigateWith=(provider:NavigationProvider)=>{const destination=navigationPlace&&navigationDestination(navigationPlace);if(!destination)return;track('NAVIGATION_HANDOFF',tripSession.id,{provider,entityId:analyticsPlaceKey(navigationPlace)});const gps=getSessionLocation();const origin=gps&&isOperationalLocation(gps)?{latitude:gps.latitude!,longitude:gps.longitude!}:undefined;launchNavigation(navigationTarget(provider,destination,origin),{mobile:isMobileNavigation(navigator.userAgent)});setNavigationPlace(null)};
  ...
  if(!regionalRuntime.nearbyEnabled)return <div className="nearby-discovery"><section className="card"><small className="eyebrow">주변 즐길거리 찾기</small><h1>{liveRegionName(region.id, region.regionName, language)} 주변 정보 준비 중</h1><p className="text-muted">현재 등록된 {liveRegionName(region.id, region.regionName, language)} 장소의 정확한 위치 정보를 확인하고 있습니다.</p></section></div>;
  return <div className="nearby-discovery">
    <section className="card"><small className="eyebrow">내 주변 찾기</small><h1>{anchor?`${anchor.label} 주변 장소`:operationalLocation?.label?`현재 ${operationalLocation.label} 주변을 찾고 있어요`:'현재 위치 주변에서 찾아드릴까요?'}</h1>{preset?.id==='nearby'&&<p className="quick-start-entry-message" role="status">{preset.entryMessage}</p>}<p className="text-muted">{searchRegionId&&searchRegionId!==region.id?`${liveRegionName(region.id, region.regionName, language)} 여행 맥락은 유지하면서 현재 위치 기준으로 보여드립니다.`:'위치를 선택하면 실제 거리순으로 보여드립니다.'}</p></section>
    ...
    <section className="card nearby-taxonomy" aria-labelledby="nearby-taxonomy-title">
      ...
      {region.id==='gyeryong'&&category==='PUBLIC_TOILET'&&<div className="nearby-lodging-actions loo-specialist-link"><div><strong>{language==='en'?'Gyeryong Public Restrooms':'계룡 공중화장실'}</strong><p>{language==='en'?'Official July 2020 records appear with current nearby results. Please verify current operation and exact location.':'계룡시 제공 2020년 7월 자료를 주변 결과에 함께 표시합니다. 현재 운영 여부와 정확한 위치는 다시 확인해 주세요.'}</p></div><a href="https://loo.exko.kr/" target="_blank" rel="noopener noreferrer">{language==='en'?'Open Specialist Map':'전문 지도 함께 보기'}</a></div>}
    ...
    {!loading && !error && origin && <>
      ...
      {selected && <section className="card nearby-detail"><div className="nearby-title-row"><div><small>{selected.categoryLabel}</small><h2 translate="no">{selected.name}</h2></div>{selected.canonicalEntityUri && <span className="badge">등록 장소 연결</span>}</div>
        ...
        {selected.canonicalEntityUri&&<VerifiedChannelActions key={`${region.id}:${selected.canonicalEntityUri}`} regionId={region.id} placeKey={selected.canonicalEntityUri}/>}<div className="nearby-actions"><a className="btn btn-primary" href={mapLink || selected.placeUrl} target="_blank" rel="noopener noreferrer" onClick={()=>track('MAP_OPENED',tripSession.id,{category:selected.category})}>지도에서 보기</a>{selected.phone&&<a className="btn btn-text" onClick={()=>track('PHONE_HANDOFF',tripSession.id,{entityId:analyticsPlaceKey(selected)})} href={`tel:${selected.phone.replace(/[^0-9+]/g,'')}`}>전화하기</a>}{navigationDestination(selected)&&<button className="btn btn-text" onClick={()=>setNavigationPlace(selected)}>길찾기</button>}<button className="btn btn-text" onClick={()=>addToItinerary(selected)}>{isLodgingCategory(selected.category)?'내 여행 숙소로 저장':'내 여행에 담기'}</button></div>
      </section>}
      ...
      <section className="card"><h2>{nearbyLabel(category)} 목록{coverageStatus==='COMPLETE'?` · ${radius/1000}km`:' · 일부 범위 확인'}</h2>{searchedAt&&<><p className="nearby-result-summary" role="status">{coverageStatus==='PARTIAL'?'일부 범위까지 확인했습니다. 확인된 장소만 보여드립니다.':expanded?`선택지를 넓히기 위해 주변 ${radius/1000}km까지 함께 찾았습니다.`:tourismSections?'지역에서 확인된 관광자원과 가까운 장소를 함께 찾았습니다.':`가까운 ${nearbyLabel(category)}부터 찾았습니다.`}</p><p className="text-muted">확인 시각 {new Date(searchedAt).toLocaleTimeString(language === 'en' ? 'en-US' : 'ko-KR',{hour:'2-digit',minute:'2-digit'})}</p></>}{places.length === 0&&!distanceBands.length ? <div><p className="text-muted">현재 조건에서 확인된 장소가 없습니다.</p><p className="nearby-range-guidance">검색 반경을 넓히거나 기준 장소를 변경해 보세요.</p>{isFoodCategory(category)&&category!=='FOOD'&&<p className="nearby-range-guidance">다른 음식 종류로 바꾸지 않았습니다. 범위를 넓히거나 ‘전체 음식점’을 선택해 보세요.</p>}<Link className="btn btn-outline" to={localizedRegionalPath('/concierge?mode=now',region.id)} state={{tripMode:'NOW',initialMessage:language==='en'?`Help me find another ${nearbyLabel(category)} option for my current situation.`:`현재 상황에 맞는 ${nearbyLabel(category)}의 다른 선택을 도와주세요.`,autoSubmit:true}}>{language==='en'?'Ask AI for Another Option':'AI 여행도우미에게 다른 선택 요청'}</Link></div> : distanceBands.length?distanceBands.map(band=><div className="nearby-result-section nearby-distance-band" key={band.id} data-distance-band={band.id}><h3>{band.label}</h3>{places.filter(place=>place.distanceBandId===band.id).length?places.filter(place=>place.distanceBandId===band.id).map(resultButton):<p className="nearby-range-guidance">{coverageStatus==='PARTIAL'?'외부 검색이 일부 완료되지 않아 이 거리 구간을 모두 확인하지 못했습니다.':'이 거리 구간에서는 확인된 장소가 없습니다.'}</p>}</div>):tourismSections?<>{tourismSections.representative.length>0&&<div className="nearby-result-section"><h3>{tourismRepresentativeTitle(liveRegionName(region.id, region.regionName, language))}</h3><p className="nearby-range-guidance">승인·검증된 지역 관광 데이터를 먼저 보여드립니다.</p>{tourismSections.representative.map(resultButton)}</div>}{tourismSections.nearby.length>0&&<div className="nearby-result-section"><h3>현재 위치에서 가까운 곳</h3><p className="nearby-range-guidance">관광 분류와 지역·좌표가 확인된 주변 검색 결과입니다.</p>{tourismSections.nearby.map(resultButton)}</div>}</>:<><div className="nearby-result-section"><h3>{distanceTrusted?'가까운 곳':'확인된 장소'}</h3>{nearbyPlaces.map(resultButton)}</div>{widerPlaces.length>0&&<div className="nearby-result-section"><h3>함께 살펴볼 만한 곳</h3>{widerPlaces.map(resultButton)}</div>}</>}{nextRadius&&<div className="nearby-range-action"><p className="nearby-range-guidance">현재 {radius/1000}km 안에서 지역·분류·좌표가 확인된 장소를 보여드립니다.</p><button type="button" className="btn btn-outline" disabled={loading} onClick={()=>setRequestedRadius(nextRadius)}>더 넓게 찾아보기 · {nextRadius/1000}km</button></div>}</section>
