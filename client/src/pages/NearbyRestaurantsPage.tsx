...
export default function NearbyRestaurantsPage() {
  ...
  const resetSearch=()=>{setRequestedRadius(undefined);setRadius(1000);setNextRadius(undefined);setExpanded(false);setCoverageStatus('COMPLETE')};
  const resultButton=(place:NearbyPlace)=><button type="button" className="nearby-result" key={`${place.provider}:${place.providerPlaceId}`} onClick={() => choose(place)} aria-label={`${place.name}, ${place.categoryLabel}${distanceTrusted&&place.distanceMeters!=null?`, ${place.distanceMeters}미터`:""}`}><span><b>{place.name}</b><small>{place.administrativeRegion?`${place.administrativeRegion} · `:""}{place.categoryLabel} · 영업 여부 확인 필요</small>{place.matchedKeyword&&<small>검색 근거: {place.matchedKeyword}</small>}</span>{distanceTrusted && place.distanceMeters != null && <strong>{place.distanceMeters < 1000 ? `${place.distanceMeters}m` : `${(place.distanceMeters / 1000).toFixed(1)}km`}</strong>}</button>;
