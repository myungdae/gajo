...
export default function NearbyRestaurantsPage() {
  ...
  useEffect(() => {
    ...
    fetchNearbyDiscovery(category, origin[0], origin[1], { radius:requestedRadius,useDistance: distanceTrusted, transportMode: 'car', weather, regionId: region.id })
      .then(response => {if(current){setPlaces(response.results);setSearchedAt(response.searchedAt);setRadius(response.radius);setNearRadius(response.initialRadius);setNextRadius(response.nextRadius);setExpanded(response.expanded)}})
  ...
  const resetSearch=()=>{setRequestedRadius(undefined);setRadius(1000);setNextRadius(undefined);setExpanded(false)};
  const resultButton=(place:NearbyPlace)=><button type="button" className="nearby-result" key={place.id} onClick={() => choose(place)} aria-label={`${place.name}, ${place.categoryLabel}${distanceTrusted&&place.distanceMeters!=null?`, ${place.distanceMeters}미터`:""}`}><span><b>{place.name}</b><small>{place.categoryLabel} · 영업 여부 확인 필요</small>{place.matchedKeyword&&<small>검색 근거: {place.matchedKeyword}</small>}</span>{distanceTrusted && place.distanceMeters != null && <strong>{place.distanceMeters < 1000 ? `${place.distanceMeters}m` : `${(place.distanceMeters / 1000).toFixed(1)}km`}</strong>}</button>;
  ...
  return <div className="nearby-discovery">
    ...
    {!loading && !error && origin && <>
      ...
      <section className="card"><h2>{nearbyLabel(category)} 목록 · {radius/1000}km</h2>{searchedAt&&<><p className="nearby-result-summary" role="status">{expanded?`선택지를 넓히기 위해 주변 ${radius/1000}km까지 함께 찾았습니다.`:`가까운 ${nearbyLabel(category)}부터 찾았습니다.`}</p><p className="text-muted">확인 시각 {new Date(searchedAt).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'})}</p></>}{places.length === 0 ? <div><p className="text-muted">현재 조건에서 확인된 장소가 없습니다.</p><p className="nearby-range-guidance">검색 반경을 넓히거나 기준 장소를 변경해 보세요.</p>{isFoodCategory(category)&&category!=='FOOD'&&<p className="nearby-range-guidance">다른 음식 종류로 바꾸지 않았습니다. 범위를 넓히거나 ‘전체 음식점’을 선택해 보세요.</p>}</div> : <><div className="nearby-result-section"><h3>{distanceTrusted?'가까운 곳':'확인된 장소'}</h3>{nearbyPlaces.map(resultButton)}</div>{widerPlaces.length>0&&<div className="nearby-result-section"><h3>함께 살펴볼 만한 곳</h3>{widerPlaces.map(resultButton)}</div>}</>}{nextRadius&&<div className="nearby-range-action"><p className="nearby-range-guidance">현재 {radius/1000}km 안에서 지역·분류·좌표가 확인된 장소를 보여드립니다.</p><button type="button" className="btn btn-outline" disabled={loading} onClick={()=>setRequestedRadius(nextRadius)}>더 넓게 찾아보기 · {nextRadius/1000}km</button></div>}</section>
