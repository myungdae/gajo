...
export default function NearbyRestaurantsPage() {
  ...
  useEffect(() => {
    ...
    fetchNearbyDiscovery(category, origin[0], origin[1], { radius,useDistance: distanceTrusted, transportMode: 'car', weather, regionId: region.id })
      .then(response => {if(current){setPlaces(response.results);setSearchedAt(response.searchedAt)}})
  ...
  return <div className="nearby-discovery">
    ...
    {!loading && !error && origin && <>
      ...
      <section className="card"><h2>{nearbyLabel(category)} 목록 · {radius/1000}km</h2>{searchedAt&&<><p className="nearby-result-summary" role="status">가까운 {nearbyLabel(category)}부터 찾았습니다.</p><p className="text-muted">확인 시각 {new Date(searchedAt).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'})}</p></>}{places.length === 0 ? <div><p className="text-muted">현재 조건에서 확인된 장소가 없습니다.</p><p className="nearby-range-guidance">검색 반경을 넓히거나 기준 장소를 변경해 보세요.</p>{isFoodCategory(category)&&category!=='FOOD'&&<p className="nearby-range-guidance">다른 음식 종류로 바꾸지 않았습니다. 범위를 넓히거나 ‘전체 음식점’을 선택해 보세요.</p>}{radius<3000&&<button type="button" className="btn btn-outline" onClick={()=>setRadius(3000)}>3km로 넓혀 찾기</button>}{radius>=3000&&radius<5000&&<button type="button" className="btn btn-outline" onClick={()=>setRadius(5000)}>5km로 넓혀 찾기</button>}</div> : places.map(place => <button type="button" className="nearby-result" key={place.id} onClick={() => choose(place)} aria-label={`${place.name}, ${place.categoryLabel}${distanceTrusted&&place.distanceMeters!=null?`, ${place.distanceMeters}미터`:""}`}><span><b>{place.name}</b><small>{place.categoryLabel} · 영업 여부 확인 필요</small>{place.matchedKeyword&&<small>검색 근거: {place.matchedKeyword}</small>}</span>{distanceTrusted && place.distanceMeters != null && <strong>{place.distanceMeters < 1000 ? `${place.distanceMeters}m` : `${(place.distanceMeters / 1000).toFixed(1)}km`}</strong>}</button>)}</section>
