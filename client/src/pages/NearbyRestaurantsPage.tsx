  ...
  return <div className="nearby-discovery">
    ...
    {!loading && !error && origin && <>
      ...
      {selected && <section className="card nearby-detail"><div className="nearby-title-row"><div><small>{selected.categoryLabel}</small><h2 translate="no">{selected.name}</h2></div>{selected.canonicalEntityUri && <span className="badge">등록 장소 연결</span>}</div>
        ...
        <div className="nearby-actions"><a className="btn btn-primary" href={mapLink || selected.placeUrl} target="_blank" rel="noopener noreferrer" onClick={()=>track('MAP_OPENED',tripSession.id,{category:selected.category})}>지도에서 보기</a>{selected.phone&&<a className="btn btn-text" onClick={()=>track('PHONE_HANDOFF',tripSession.id,{entityId:analyticsPlaceKey(selected)})} href={`tel:${selected.phone.replace(/[^0-9+]/g,'')}`}>전화하기</a>}{navigationDestination(selected)&&<button className="btn btn-text" onClick={()=>setNavigationPlace(selected)}>길찾기</button>}<button className="btn btn-text" onClick={()=>addToItinerary(selected)}>{isLodgingCategory(selected.category)?'내 여행 숙소로 저장':'내 여행에 담기'}</button></div>
      </section>}
