...
export default function NearbyRestaurantsPage() {
  ...
  return <div className="nearby-discovery">
    ...
    <section className="card nearby-taxonomy" aria-labelledby="nearby-taxonomy-title">
      ...
      {region.id==='gyeryong'&&category==='PUBLIC_TOILET'&&<div className="nearby-lodging-actions loo-specialist-link"><div><strong>{language==='en'?'Find a Public Restroom':'공중화장실 바로 찾기'}</strong><p>{language==='en'?'Open the dedicated location-based restroom map.':'위치 기반 공중화장실 전문 지도를 엽니다.'}</p></div><a href="https://loo.exko.kr/" target="_blank" rel="noopener noreferrer">{language==='en'?'Open Restroom Map':'공중화장실 지도 열기'}</a></div>}
