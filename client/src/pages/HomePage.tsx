...
export default function HomePage(){
  ...
  return <div className="home-page home-conversation-first" style={{'--region-accent':region.accent}as React.CSSProperties}>
    ...
    <section className="home-arrival" aria-labelledby="home-arrival-title"><div><p className="eyebrow">여행 중 필요한 도움</p><h2 id="home-arrival-title"><span>{region.regionName}에 오신 것을 환영합니다.</span><span>지금 무엇을 도와드릴까요?</span></h2><p>현재 위치와 여행 상황에 맞춰 먹을 곳·쉴 곳·볼 곳·숙소 등을 찾아드립니다.</p></div><div className="home-arrival-actions" aria-label={`${region.regionName} 여행 중 필요한 도움`}><button type="button" onClick={()=>findNearby('FOOD')}>지금 밥 먹고 싶어요</button><button type="button" onClick={()=>askNow('카페에서 쉬고 싶어요.')}>카페에서 쉬고 싶어요</button><button type="button" onClick={()=>findNearby('TOURIST_ATTRACTION')}>주변 관광지 보고 싶어요</button><button type="button" onClick={()=>askNow('지금 상황에서 다음에 갈 곳을 추천해 주세요.')}>다음에 갈 곳 추천해 주세요</button><button type="button" onClick={()=>findNearby('LODGING')}>숙소를 찾고 싶어요</button></div><small>위치 권한은 주변 찾기에서 ‘현재 위치로 찾기’를 누를 때만 요청합니다.</small></section>
