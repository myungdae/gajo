...
export default function AdminPage({adminToken}:{adminToken:string}) {
...
<RegionalDataManager key={`data:${region.id}`} adminToken={adminToken} />
      ...
      {pilot?.sessionsByRegion&&<div className="card"><h2>지역별 이용</h2><p>{pilot.sessionsByRegion.map((item:any)=>`${item.label===region.id?region.regionName:item.label} ${item.total}`).join(' · ')||'아직 없음'}</p></div>}
